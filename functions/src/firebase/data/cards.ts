...
/**
 * Return an array of {@link ResponseCard} objects that were searched for
 * via their index hash across all sets.
 *
 * CAVEAT: Do to how the cards are hashed this may return cards from other
 * sets then they were originally intended to be sourced from since the
 * index is just `sha256(card.text)`. In the future, we should probably
 * adjust this algorithm to be `sha256(set.id + card.text)`. This should
 * really have an impact on gameplay since the player won't be aware of
 * where their cards are from, but it could impact how we are managing id's.
 *
 * @param {string[]} ids
